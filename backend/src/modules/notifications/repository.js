...
async function get(userId, { page = 1, limit = 20 } = {}) {
  ...
  const countRes = await pool.query('SELECT COUNT(*) FROM notifications WHERE user_id = $1 AND deleted_at IS NULL', [userId]);
  return {
    data: res.rows,
...
async function markAllRead(userId) {
  await pool.query('UPDATE notifications SET read = TRUE WHERE user_id = $1 AND read = FALSE AND deleted_at IS NULL', [userId]);
}

async function deleteNotification(notificationId, userId) {
  await pool.query('UPDATE notifications SET deleted_at = NOW() WHERE id = $1 AND user_id = $2', [notificationId, userId]);
}

async function getUnreadCount(userId) {
  const res = await pool.query('SELECT COUNT(*) FROM notifications WHERE user_id = $1 AND read = FALSE AND deleted_at IS NULL', [userId]);
