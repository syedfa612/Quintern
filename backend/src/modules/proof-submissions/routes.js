...
async function routes(fastify) {
  ...
  fastify.post('/submit', { preHandler: [auth, rbac('INTERN')] }, async (req, reply) => {
    ...
    if (!data) return reply.status(400).send({ error: 'Image file required' });

    if (!ALLOWED_MIMES.includes(data.mimetype)) {
      return reply.status(400).send({ error: 'Only JPEG, PNG, GIF images are allowed' });
    }
  ...
  fastify.get('/task/:taskId', { preHandler: [auth, rbac('CAPTAIN', 'TL', 'SENIOR_TL', 'ADMIN')] }, async (req, reply) => {
    const pool = require('../../config/db');
    ...
    if (req.user.role !== 'ADMIN') {
      const ownsTask = await checkHierarchyAccess(req.user.id, task.created_by);
      if (!ownsTask) return reply.status(403).send({ error: 'Task is not in your hierarchy' });
    }
